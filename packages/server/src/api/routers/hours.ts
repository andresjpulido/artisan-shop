...
export default (app: Router) => {
  ...
  app.get("/hours/:username/:isPaid", HoursCtrl.getHoursByUserUsername);
  ...
  app.get("/hours", async (req, res, next) => {
    ...
    const list = await serviceInstance.get(query);
    return res.json(list);
  });
  app.get("/hours/:id", async (req, res, next) => {
    const serviceInstance = Container.get(hourService);
    const id = req.params.id;
    const list = await serviceInstance.getById(id);
    return res.json(list);
  });
  ...
  app.delete("/hours", async (req, res, next) => {
    ...
    const list = await serviceInstance.deleteCollection(ids);
    return res.json(list);
  });
};
