...
import service from "../../services/payslipService";
...
export default class Payslips {
  static async create(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      newEntry = await instance.create(userObj);
    } catch (error) {
      return next(error);
    }
  ...
  static async getPayslips(req: Request, res: Response, next: NextFunction) {
  ...
  static async getPayslipsByUserId(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    ...
    try {
      const instance = Container.get(service);
      list = await instance.getAllHoursByEmployeeId(Number(id));
    } catch (error) {
      return next(error);
    }
    return res.status(200).json(list);
  ...
  static async getHoursByUserId(
    req: Request,
    res: Response,
    next: NextFunction
  ) {}

  static async getPayslipsById_(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    ...
    try {
      const userInstance = Container.get(service);
      item = await userInstance.getById(Number(id));
    } catch (error) {
      return next(error);
    }
