...
class ActionUtility {
  static invokeServicePost(dispatch, actionType, requestObj, url, urlredirect) {
    let token = localStorage.getItem("session");

    //TODO if no token return to login
    

    //TODO if token invalid return to login
    ...
    axios
      ...
      .catch(function(error) {
        ...
        if (error.response) {
          ...
          dispatch({
            type: ALERT,
            payload: { type: "ERROR", description: error.message },
          });
        }
    ...
    axios
      ...
      .catch(function(error) {
        ...
        if (error.response) {
          ...
          dispatch({
            type: ALERT,
            payload: { type: "ERROR", description: error.request },
          });
        } else {
          // Something happened in setting up the request that triggered an Error
  ...
  static invokeServicePostFormData(
    dispatch,
    actionType,
    requestObj,
    url,
    ...
    axios
      ...
      .then((response) => {
        ...
        dispatch({
          type: UPD_ORDER,
          payload: { ...order, products: order.products },
        });
        //dispatch({ type: FINISHED, payload: null })
      })
      ...
      .catch(function(error) {
        ...
        if (!error.response) {
          dispatch({
            type: SHOW_ERRORS,
            error: { message: "No connection with the server." },
          });
          dispatch({ type: FINISHED, payload: null });
        }
      });
